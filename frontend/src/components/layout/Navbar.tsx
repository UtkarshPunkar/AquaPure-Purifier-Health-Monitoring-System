...
export const Navbar: React.FC<NavbarProps> = ({ onMenuToggle }) => {
  ...
  return (
    <header className="app-header sticky top-0 z-30 px-3 sm:px-6 py-2.5 bg-white/95 dark:bg-[#0c1427]/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 transition-colors">
      <div className="flex items-center justify-between gap-4 max-w-[1750px] mx-auto">
        {/* Left: Hamburger + Page Title */}
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={onMenuToggle}
            className="p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden focus:outline-none transition-colors shrink-0"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative" ref={alertsRef}>
            <button
              ...
              className="relative p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200/80 dark:border-slate-700/60 transition-all interactive-btn cursor-pointer"
              title="System Alerts & Notifications"
              aria-label="Notifications"
            >
              <Bell size={16} />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold text-white bg-amber-500 rounded-full border-2 border-white dark:border-slate-900 shadow-xs animate-badge-ping">
            ...
            {isAlertsOpen && (
              <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden animate-scale-up origin-top-right">
          ...
          <div className="relative pl-1" ref={profileRef}>
            <button
              ...
              className="flex items-center gap-2.5 p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-all focus:outline-none interactive-btn cursor-pointer"
            ...
            <div className="max-h-80 overflow-y-auto p-2">
              {searchQuery.trim() === '' ? (
                <div className="p-4 text-center text-xs text-slate-400">
                  Type to instantly search all 5 water purifiers across campus buildings.
                </div>
