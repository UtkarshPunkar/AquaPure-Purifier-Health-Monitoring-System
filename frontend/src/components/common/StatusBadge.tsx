...
export const StatusBadge: React.FC<StatusBadgeProps> = ({
  ...
  if (['HEALTHY', 'EXCELLENT', 'OPTIMAL', 'GOOD', 'ONLINE', 'COMPLETED', 'LOW', 'CONNECTED'].includes(norm)) {
  ...
  } else if (['OFFLINE', 'DISCONNECTED'].includes(norm)) {
  ...
  return (
    ...
    >
      <span className="relative flex h-2 w-2 shrink-0">
        {showPulse && !['OFFLINE', 'COMPLETED', 'DISCONNECTED'].includes(norm) && (
          <span
